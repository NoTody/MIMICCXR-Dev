...
def get_args():
    ...
    parser.add_argument("--model_teacher_load_path", type=str, default="None")
    parser.add_argument("--model_student_load_path", type=str, default="None")
...
#                                         (lambda crops: torch.stack([crop for crop in crops]))
    ...
    if args.model_name == "densenet121":
        ...
        features_dim = 2048
        out_size = N_CLASSES
        model_teacher = densenet_model(size, features_dim, out_size, pretrained=args.pretrained).cuda()
        model_student = densenet_model(size, features_dim, out_size, pretrained=args.pretrained).cuda()
    elif args.model_name == "resnet50":
        size = 50
        features_dim = 2048
        out_size = N_CLASSES
        model_teacher = resnet_model(size, features_dim, out_size, pretrained=args.pretrained).cuda()
        model_student = resnet_model(size, features_dim, out_size, pretrained=args.pretrained).cuda()
    elif args.model_name == "vitb16":
        size = "base" 
        features_dim = 768
        out_size = N_CLASSES
        model_teacher = vit_model(size, features_dim, out_size, pretrained=args.pretrained).cuda()
        model_student = vit_model(size, features_dim, out_size, pretrained=args.pretrained).cuda()
    else:
        raise NotImplementedError("Model Not Implemented!")

    model_teacher, model_student = torch.nn.DataParallel(model_teacher).cuda(), torch.nn.DataParallel(model_student).cuda()
    criterion = torch.nn.BCELoss()
    
    if args.model_teacher_load_path != "None":
        checkpoint = torch.load(args.model_teacher_load_path)
        state_dict = {k.replace("img_backbone.", "module."): v for k, v in checkpoint['state_dict'].items()}
        load_status = model_teacher.load_state_dict(state_dict, strict=False)
        print(f"Load Status: {load_status}")
       
    if args.model_student_load_path != "None":
        checkpoint = torch.load(args.model_student_load_path)
        state_dict = {k.replace("img_backbone.", "module."): v for k, v in checkpoint['state_dict'].items()}
        load_status = model_student.load_state_dict(state_dict, strict=False)
        print(f"Load Status: {load_status}")
    ...
    if "LP" in args.method:
        ...
        optimizer_teacher = torch.optim.Adam(model_teacher.parameters(), lr=1e-4, betas=(0.9, 0.999))
        optimizer_student = torch.optim.Adam(model_student.parameters(), lr=1e-4, betas=(0.9, 0.999))
        
        best_model = train_distill(args, model_student, model_teacher, optimizer_student, optimizer_teacher, criterion, train_loader, val_loader, N_CLASSES, N_EPOCHS, CLASS_NAMES)
    ...
    if "FT" in args.method:
        print("---------------------Fine-Tuning---------------------")
        if args.method == "LPFT":
            model_student = best_model

        # unfreeze for finetuning
        for param in model_teacher.module.backbone.parameters():
            param.requires_grad = True
        for param in model_student.module.backbone.parameters():
            param.requires_grad = True
